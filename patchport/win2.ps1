...
# Will take effect after enable the br-int
Set-NetAdapterAdvancedProperty -Name br-ext -RegistryKeyword NetworkAddress -RegistryValue 005056A7769A

#3. Add uplink and gw0
ovs-vsctl add-port br-ext "Ethernet0 2" -- set interface "Ethernet0 2" ofport_request=$UplinkPort
Enable-NetAdapter -InterfaceAlias br-ext

ovs-vsctl.exe add-port br-int gw0 -- set interface gw0 type=internal ofport_request=$GwPort
...
ovs-vsctl.exe add-port br-int patch-ext -- set interface patch-ext type=patch options:peer=patch-int ofport_request=$PatchExtPort
ovs-vsctl.exe add-port br-ext patch-int -- set interface patch-int type=patch options:peer=patch-ext ofport_request=$PatchIntPort
...
ovs-ofctl add-flow br-ext "table=0,cookie=0x520,in_port=LOCAL,priority=200,ip,nw_src=192.168.187.0/24 actions=resubmit(,10)"
ovs-ofctl add-flow br-ext "table=0,cookie=0x520,in_port=LOCAL,priority=200,ip,nw_dst=192.168.187.0/24 actions=$PatchIntPort"
# CT table
...
ovs-ofctl add-flow br-ext "table=20,cookie=0x520,priority=200,in_port=$UplinkPort,ip,nw_dst=192.168.187.0/24 actions=$PatchIntPort"
...
ovs-ofctl add-flow br-ext "table=20,cookie=0x520,priority=200,in_port=$UplinkPort,ip,nw_src=10.176.26.107,nw_dst=192.168.187.0/24 actions=load:0x1->NXM_NX_PKT_MARK[],output:$PatchIntPort"
...
#  cookie=0x520, duration=8.937s, table=20, n_packets=0, n_bytes=0, priority=200,ip,in_port=Ethernet0 2,nw_src=10.176.26.107,nw_dst=192.168.187.0/24 actions=load:0x1->NXM_NX_PKT_MARK[],output:"patch-int"
